...
- Кнопка «Смотреть» на **Кинопоиске, IMDb, TMDB, Letterboxd, Rotten Tomatoes и Netflix**.
...
Netflix и TMDB работают через резолв ID в IMDb по Wikidata — тайтлов, которых
там нет (в основном новинки и региональные оригиналы Netflix), кнопка не
откроет; это ограничение источника данных, а не бага.
